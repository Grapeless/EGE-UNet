...
def _unpack_model_output(result):
    """Unpack model output which may have 2 or 3 values depending on boundary mode."""
    if len(result) == 3:
        gt_pre, out, boundary_out = result
    else:
        gt_pre, out = result
        boundary_out = None
    return gt_pre, out, boundary_out
...
def train_one_epoch(train_loader,
    ...
    for iter, data in enumerate(train_loader):
        ...
        gt_pre, out, boundary_out = _unpack_model_output(model(images))
...
def val_one_epoch(test_loader,
    ...
    with torch.no_grad():
        for data in tqdm(test_loader):
            ...
            gt_pre, out, boundary_out = _unpack_model_output(model(img))
            if boundary_out is not None:
