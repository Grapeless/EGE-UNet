"""
Boundary-Aware Supervision (BAS) Module - Improved Version

Key improvements over v1:
1. Multi-scale boundary heads (H/8 + H/2) for coarse-to-fine supervision
2. Dilated boundary GT (thicker boundaries easier to learn)
3. Boundary Dice + BCE combined loss
4. Boundary feature feedback into decoder path
"""
...
def generate_boundary_gt(mask, dilation_radius=3):
    ...
    Dilation creates a thicker boundary band that is easier to learn.
...
class BoundaryLoss(nn.Module):
    ...
    Dice handles class imbalance better than pos_weight alone.
    """
    ...
    def forward(self, pred_logits, target_boundary):
        ...
        if pred_logits.shape[-2:] != target_boundary.shape[-2:]:
            target_boundary = F.interpolate(
                ...
                mode='nearest'  # nearest for binary masks
            )
